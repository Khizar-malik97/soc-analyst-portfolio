  ...
  if (!report) {
    return (
      <section className="flex min-h-screen flex-col items-center justify-center px-6 text-center">
        ...
        <Link
          to="/reports"
          className="mt-6 inline-flex items-center gap-2 rounded-lg bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground transition hover:opacity-90"
        >
        ...
        <div className="mt-10">
          ...
          <ol className="space-y-3 border-l border-border pl-5">
            {report.incidentTimeline.map((event, i) => (
              <li key={i} className="relative text-sm text-muted-foreground">
        ...
        <div className="mt-10">
          ...
          <ul className="space-y-2">
            {report.indicatorsOfCompromise.map((ioc, i) => (
              <li
                key={i}
                className="rounded-md bg-background/60 px-4 py-2 font-mono text-xs text-muted-foreground"
              >
                {ioc}
              </li>
            ))}
